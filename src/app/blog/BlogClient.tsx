...
export default function BlogClient({ initialBlogs }: Props) {
  ...
  // Label each post by its actual WordPress category (set in the WP editor).
  // Prefer a specific category over the generic "Blog" bucket.
  ...
  // Filter tabs are built from the categories actually present in your posts.
  const categories = ['All', ...Array.from(new Set(initialBlogs.map(getPostCategory)))];
